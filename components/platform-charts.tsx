...
export function PlatformCharts({ data }: PlatformChartsProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return times.map((time, index) => {
      ...
      const multiplier = index === 3 ? 1 : index === 4 ? 0.9 : index === 2 ? 0.7 : 0.5
  ...
  const platformsWithCounts = [
    {
      ...
      key: "tiktok",
      count: isLoading ? "..." : (platformBreakdown.tiktok || 0).toLocaleString(),
    },
  ]
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-card-foreground">Comments Over Time</CardTitle>
        <div className="flex flex-wrap gap-4 mt-4">
          {platformsWithCounts.map((platform) => (
            <div key={platform.name} className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-card-foreground">{platform.count}</span>
            </div>
          ))}
        </div>
      </CardHeader>
            ...
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id="tiktok" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="oklch(0.60 0.22 280)" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="oklch(0.60 0.22 280)" stopOpacity={0} />
                </linearGradient>
              </defs>
