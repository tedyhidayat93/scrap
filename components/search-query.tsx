...
interface SearchQueryProps {
  onSearch: (query: string, type: "username" | "video" | "keyword", latestOnly?: boolean) => void
  currentQuery: string
  queryType: "username" | "video" | "keyword"
  isLoading?: boolean
}

export function SearchQuery({ onSearch, currentQuery, queryType, isLoading }: SearchQueryProps) {
  ...
  const getDisplayText = () => {
    ...
    if (queryType === "video") {
      return (
        <div className="text-sm bg-muted/50 p-4 rounded-md flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground truncate">{currentQuery}</span>
        </div>
      )
    }
    ...
    const displayUsername = currentQuery.startsWith("@") ? currentQuery : `@${currentQuery}`
    return (
      <div className="text-sm bg-muted/50 p-4 rounded-md flex items-center gap-2">
  ...
  return (
    <Card className="p-6 bg-card border-border">
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-semibold text-card-foreground mb-1">TikTok Analysis</h2>
          <p className="text-sm text-muted-foreground">Enter a keyword, username, or video URL to analyze comments</p>
        </div>
        ...
        <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-md">
          <Switch id="latest-only" checked={latestOnly} onCheckedChange={setLatestOnly} disabled={isLoading} />
          ...
          <div className="text-sm text-muted-foreground bg-muted/50 p-4 rounded-md">
            <div className="space-y-2">
              ...
              <ul className="list-disc list-inside space-y-1 ml-2">
                ...
                <li>Video URL: https://www.tiktok.com/@user/video/7563193719487319304</li>
              </ul>
            </div>
          </div>
        )}
