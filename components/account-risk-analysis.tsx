...
export function AccountRiskAnalysis({ data }: AccountRiskAnalysisProps) {
    ...
    return (
      <Card className="bg-card border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold text-card-foreground">Account Risk Analysis</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-center py-4 text-sm text-muted-foreground">Analyzing accounts...</div>
        </CardContent>
      </Card>
    )
  }
  ...
  const accountRisks: AccountRisk[] = Array.from(accountMap.entries())
    .map(([userId, userComments]) => {
      const username = userComments[0]?.user?.unique_id || userComments[0]?.user?.nickname || "Unknown User"
      const avatar = userComments[0]?.user?.avatar_thumb?.url_list?.[0] || "/placeholder.svg"

      // Calculate risk factors
      const commentCount = userComments.length
      const botProbability = userComments.filter((c) => c.isBot).length / commentCount
      const negativeComments = userComments.filter((c) => c.sentiment === "negative").length
      const negativeRatio = negativeComments / commentCount

      // Calculate average comment length
      const avgCommentLength = userComments.reduce((sum, c) => sum + (c.text?.length || 0), 0) / commentCount
      ...
      const uniqueTexts = new Set(userComments.map((c) => c.text?.toLowerCase().trim()))
  ...
  const criticalCount = accountRisks.filter((a) => a.riskLevel === "critical").length
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5 text-xs">
            <Badge variant="outline" className="bg-red-500/10 text-red-500 border-red-500/20 px-2 py-0.5">
              {criticalCount}
            </Badge>
            <Badge variant="outline" className="bg-orange-500/10 text-orange-500 border-orange-500/20 px-2 py-0.5">
              {highCount}
            </Badge>
            <Badge variant="outline" className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 px-2 py-0.5">
              {mediumCount}
            </Badge>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-center space-y-1 w-full">
                    <p className="text-xs font-medium text-card-foreground truncate">{account.username}</p>
                    <Badge variant="outline" className={`${riskLevelColors[account.riskLevel]} text-xs px-1.5 py-0`}>
                      <Icon className="w-2.5 h-2.5 mr-0.5" />
                      {account.riskScore}
                    </Badge>
                    <p className="text-xs text-muted-foreground">{account.commentCount} comments</p>
                    {account.spamIndicators.length > 0 && (
                      <div className="flex flex-wrap gap-0.5 justify-center">
                        {account.spamIndicators.slice(0, 2).map((indicator, idx) => (
                          <Badge
                            key={idx}
                            variant="outline"
