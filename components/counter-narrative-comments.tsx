...
import { AlertTriangle, ThumbsDown, User, Sparkles, RefreshCw } from "lucide-react"
...
export function CounterNarrativeComments({ data }: CounterNarrativeCommentsProps) {
  ...
  const counterPercentage = aiCounterData?.summary?.percentage?.toFixed(1) || "0"
          ...
          <div className="flex items-center gap-2">
            {aiCounterData && (
              <Badge variant="outline" className="bg-chart-5/10 text-chart-5 border-chart-5/20">
                {aiCounterData.counterComments?.length || 0} comments ({counterPercentage}%)
              </Badge>
            )}
        ...
        {isLoading || isAnalyzing ? (
          <div className="h-[200px] flex items-center justify-center text-muted-foreground">
            <div className="flex items-center gap-2">
              ...
              {isAnalyzing ? "AI is analyzing counter-narratives..." : "Loading comments..."}
            </div>
          </div>
        ) : error ? (
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-card-foreground">
                    {aiCounterData.summary.totalCounter} comments ({counterPercentage}% of total) express disagreement
                  </p>
                  {aiCounterData.summary.mainObjections && aiCounterData.summary.mainObjections.length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs font-medium text-muted-foreground mb-1">Common objections:</p>
                      <ul className="text-xs text-muted-foreground space-y-1">
                        {aiCounterData.summary.mainObjections.map((objection: string, idx: number) => (
            ...
            <div className="space-y-3">
              {aiCounterData.counterComments.map((comment: any, index: number) => (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                        ...
                        {comment.create_time && (
                          <span>{formatDistanceToNow(new Date(comment.create_time * 1000), { addSuffix: true })}</span>
                        )}
                        {comment.counterKeywords && comment.counterKeywords.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {comment.counterKeywords.map((keyword: string) => (
