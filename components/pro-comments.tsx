...
export function ProComments({ data }: ProCommentsProps) {
  ...
  const displayData = aiProData || {
    ...
    summary: {
      ...
      percentage:
        comments.length > 0
          ? (basicProComments.length / comments.length) * 100
          : 0,
      commonThemes: [],
    },
  };

  const proPercentage = displayData?.summary?.percentage
      ...
      <CardContent>
        {isLoading || isAnalyzing ? (
          ...
          <div className="text-sm text-muted-foreground text-center py-8">
            {handle
              ? "No pro comments detected"
              : "Enter a query to analyze pro comments"}
          </div>
        ) : (
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-card-foreground">
                    {displayData.summary.totalPro} comments ({proPercentage}% of
                    total) support the video
                  </p>
                  {displayData.summary.commonThemes &&
                    displayData.summary.commonThemes.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium text-muted-foreground mb-1">
                          Common themes:
                        </p>
                        <ul className="text-xs text-muted-foreground space-y-1">
                          {displayData.summary.commonThemes.map(
