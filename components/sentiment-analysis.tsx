...
export function SentimentAnalysis({ data }: SentimentAnalysisProps) {
  ...
  const displayKeywords = hasAnalyzed && aiKeywords.length > 0 ? aiKeywords : basicKeywords

  return (
    <Card className="bg-card border-border">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-card-foreground">Sentiment Analysis</CardTitle>
          {comments.length > 0 && (
            <Button variant="outline" size="sm" onClick={analyzeKeywords} disabled={loadingKeywords}>
              {loadingKeywords ? (
                <RefreshCw className={`h-4 w-4 mr-2 animate-spin`} />
              ) : (
                <Sparkles className={`h-4 w-4 mr-2 ${loadingKeywords ? "animate-pulse" : ""}`} />
              )}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="space-y-2">
                {loadingKeywords ? (
                  <div className="text-sm text-muted-foreground text-center py-4">Analyzing keywords with AI...</div>
                ) : displayKeywords.length > 0 ? (
                  displayKeywords.map((keyword) => (
                    <div key={keyword.word} className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-card-foreground font-medium">{keyword.word}</span>
                  ...
                  <div className="text-sm text-muted-foreground text-center py-4">
                    {handle ? "No keywords found" : "Enter a query to see keywords"}
                  </div>
                )}
              </div>
            </div>
