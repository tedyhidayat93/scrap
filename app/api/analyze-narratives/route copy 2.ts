...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      // fallback jika ollama gagal arahkan ke openai 
...
// ... existing code ...
    ...
    try {
      ...
      const result = narrativeSchema.safeParse(JSON.parse(response));
      ...
      try {
        console.log("[v0] Falling back to OpenAI...");
        ...
        console.error("[v0] Error with OpenAI fallback:", aiError);
        ...
        // If both Ollama and OpenAI fail, return the original error
        return Response.json(
          { 
