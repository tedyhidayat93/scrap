...
function parseTikTokUrl(url: string): { username: string; videoId: string } | null {
  try {
...
async function fetchTikTokUserVideos(handle: string) {
  try {
    ...
    if (!response.ok) {
      console.error("[v0] TikTok videos API error:", response.status, responseText)
    ...
    try {
      ...
      return { error: `Invalid JSON response: ${responseText.substring(0, 100)}`, status: 500 }
    }
  } catch (error) {
...
async function fetchTikTokVideoComments(videoUrl: string, cursor?: number) {
  try {
    const url = cursor
      ? `${BASE_URL}/tiktok/video/comments?url=${encodeURIComponent(videoUrl)}&count=130&cursor=${cursor}`
      : `${BASE_URL}/tiktok/video/comments?url=${encodeURIComponent(videoUrl)}&count=130`
    ...
    if (!response.ok) {
      console.error("[v0] TikTok comments API error:", response.status, responseText)
      return { error: responseText, status: response.status }
    }
...
async function fetchTikTokVideoInfo(videoUrl: string) {
  try {
    ...
    if (!response.ok) {
      console.error("[v0] TikTok video info API error:", response.status, responseText)
      return { error: responseText, status: response.status }
    }
...
async function searchTikTokByKeyword(keyword: string, cursor?: number) {
  try {
    ...
    const url = cursor
      ? `${BASE_URL}/tiktok/search/keyword?query=${encodeURIComponent(cleanKeyword)}&count=30&cursor=${cursor}`
      : `${BASE_URL}/tiktok/search/keyword?query=${encodeURIComponent(cleanKeyword)}&count=30`
    ...
    if (!response.ok) {
      console.error("[v0] TikTok search API error:", response.status, responseText)
      return { error: responseText, status: response.status }
    }
    ...
    try {
      ...
      console.log("[v0] search_item_list is array:", Array.isArray(data.search_item_list))

      if (data.search_item_list && Array.isArray(data.search_item_list)) {
        console.log("[v0] search_item_list length:", data.search_item_list.length)
        if (data.search_item_list.length > 0) {
          console.log("[v0] First search_item keys:", Object.keys(data.search_item_list[0]))
          console.log("[v0] First search_item sample:", JSON.stringify(data.search_item_list[0]).substring(0, 200))
        }
      }

      if (data.aweme_list && typeof data.aweme_list === "object") {
        console.log("[v0] aweme_list keys:", Object.keys(data.aweme_list))
        console.log("[v0] aweme_list sample:", JSON.stringify(data.aweme_list).substring(0, 200))
      }
...
export async function GET(request: Request) {
  ...
  if (type !== "username" && type !== "video" && type !== "keyword") {
    return NextResponse.json(
      {
        success: false,
        error: "Invalid type parameter. Must be 'username', 'video', or 'keyword'",
      },
      { status: 400 },
    )
  }
  ...
  try {
    if (type === "keyword") {
      ...
      while (pageCount < maxPages && allVideos.length < (latestOnly ? 1 : 100)) {
        const searchResult = await searchTikTokByKeyword(keyword, currentCursor)
        ...
        const videos = searchItems.filter((item: any) => item.aweme_info).map((item: any) => item.aweme_info)

        console.log("[v0] Page", pageCount + 1, "- Videos found:", videos.length)
      ...
      if (allVideos.length === 0) {
        return NextResponse.json({
          ...
          debug: {
            keyword: keyword,
            message: "The API returned successfully but no videos were found. Try a different keyword.",
          },
        })
      }

      const videosToAnalyze = latestOnly ? allVideos.slice(0, 1) : allVideos.slice(0, 100)
      ...
      for (const video of videosToAnalyze) {
        try {
          ...
          if (!commentsResult.error && commentsResult.data?.comments) {
            ...
            successfulVideos++
            console.log("[v0] Successfully fetched comments from video:", videoUrl)
          } else {
            failedVideos++
            console.log("[v0] Failed to fetch comments from video:", videoUrl, "Error:", commentsResult.error)
          }
        } catch (videoError) {
          failedVideos++
          console.error("[v0] Unexpected error fetching video comments:", videoError)
        }
      ...
      console.log("[v0] Successful videos:", successfulVideos, "Failed videos:", failedVideos)
      ...
      if (allComments.length === 0) {
        return NextResponse.json({
          ...
          debug: {
            ...
            successfulVideos,
            failedVideos,
            message: "All video comment fetches failed. Try a different keyword.",
          },
        })
      }
      ...
      const uniqueUsers = new Set(processedComments.map((c) => c.user?.unique_id || c.user?.id)).size
    ...
    if (type === "video") {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          hasMore,
          videoStats,
          videos: videoInfoResult.data?.itemInfo?.itemStruct ? [videoInfoResult.data.itemInfo.itemStruct] : [],
        },
      })
    }
    ...
    if (videos.length === 0) {
      return NextResponse.json({
        ...
        debug: {
          handle: handle,
          message: "The API returned successfully but no videos were found. Check if the handle is correct.",
        },
      })
    }

    const videosToAnalyze = latestOnly ? videos.slice(0, 1) : videos.slice(0, 100)
    const allComments: any[] = []
    let successfulVideos = 0
    let failedVideos = 0
    ...
    if (allComments.length === 0) {
      return NextResponse.json({
        ...
        debug: {
          ...
          successfulVideos,
          failedVideos,
          message: "All video comment fetches failed. The videos might be private or the API might be rate limiting.",
        },
      })
    }
